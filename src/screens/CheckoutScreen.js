...
const CheckoutScreen = ({ route, navigation }) => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      Alert.alert('Order Failed', error.response?.data?.error || 'Something went wrong.');
    } finally {
      setProcessing(false);
    }
  };
    ...
    >
      <View style={styles.container}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.receiptCard}>
              {Object.values(cart).map((item) => (
                <View key={item.id} style={styles.receiptRow}>
                  <View style={styles.itemBadge}><Text style={styles.itemQty}>{item.quantity}x</Text></View>
          ...
          {/* Delivery Note (NEW) */}
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Where are you parked? (Optional)</Text>
            <View style={styles.noteContainer}>
              <MaterialCommunityIcons name="comment-text-outline" size={20} color="#FF5722" style={styles.noteIcon} />
              <TextInput
        ...
        <View style={styles.footer}>
          <TouchableOpacity style={styles.placeOrderBtn} onPress={handlePlaceOrder} disabled={processing}>
            {processing ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>PLACE ORDER</Text>}
          </TouchableOpacity>
        </View>
      </View>
    </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 15, fontWeight: 'bold', color: '#444', marginBottom: 10, marginLeft: 5 },
  receiptCard: { backgroundColor: '#fff', borderRadius: 15, padding: 15, elevation: 3 },
  receiptRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 10 },
  itemBadge: { backgroundColor: '#FFF3E0', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 5, marginRight: 10 },
  ...
  // Note Styles
  noteContainer: { flexDirection: 'row', backgroundColor: '#fff', borderRadius: 12, padding: 12, elevation: 2, minHeight: 60 },
  noteIcon: { marginTop: 2 },
  noteInput: { flex: 1, marginLeft: 10, fontSize: 14, color: '#333', textAlignVertical: 'top' },

  carsList: { paddingRight: 20 },
  carCard: { width: 120, height: 130, backgroundColor: '#fff', borderRadius: 12, padding: 10, marginRight: 12, alignItems: 'center', justifyContent: 'center', elevation: 2, borderWidth: 2, borderColor: 'transparent' },
  ...
  addCarBtn: { width: 80, height: 130, borderRadius: 12, borderStyle: 'dashed', borderWidth: 2, borderColor: '#ddd', alignItems: 'center', justifyContent: 'center' },
  paymentOption: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', padding: 15, borderRadius: 12, elevation: 2 },
  ...
  placeOrderBtn: { backgroundColor: '#FF5722', height: 55, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
